...
#need to add parallelization once the manager-worker system is improved, should be GPU accelerated
...
def get_pod_modes():
    ...
    for i in range(len(flp)):
        ...
        ############debug  
        X_np = X.cpu().numpy()
        max_node_jump = np.max(np.abs(np.diff(X_np, axis=1)), axis=1)
        top = np.argsort(-max_node_jump)[:10]
        print("top nodes, max jumps:", [(int(i), float(max_node_jump[i])) for i in top])
        Phi = modes[:, :3]       # N x m
        a = (Phi.T * node_weights_t[None,:]) @ X  # m x Nt
        for k in range(a.shape[0]):
            diffs = np.abs(np.diff(a[k].cpu().numpy()))
            print(f"mode {k}: max jump = {diffs.max():.3e}, mean jump = {diffs.mean():.3e}")
        #######################
